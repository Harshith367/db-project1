...
//      case BPTREE_MAP  -> new BpTreeMap <> (KeyType.class, Comparable [].class);
    ...
    public Table project (String attributes)
    {
        ...
        for (Comparable[] t : tuples) {
            //creating a new row for each iteration
            Comparable[] row = new Comparable[attrs.length];
            int i = 0;
            for (String attr : attrs) {
                //converting the attribute array of strings into list and finding index of each attribute.
                List stringlist=new ArrayList<>(Arrays.asList(attribute));
                int j = stringlist.indexOf(attr);
                if (j < 2) {
                    row[i] = t[j];
                    i++;
                }
